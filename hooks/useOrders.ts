...
import {
  fetchMyOrders, createOrder, cancelOrder,
  fetchAllOrders, fetchOrderById, updateOrderStatus, clearOrderError,
...
export function useOrders() {
  ...
  const { myOrders, allOrders, current, isLoading, isCreating, error, meta } =
    useAppSelector((s) => s.orders);
  const { user } = useAppSelector((s) => s.auth);

  useEffect(() => {
    if (user) dispatch(fetchMyOrders(undefined));
  }, [user, dispatch]);
  ...
  const fetchAll = useCallback(
    (params?: Record<string, string>) => { dispatch(fetchAllOrders(params)); },
    [dispatch],
  );
  ...
  return {
    myOrders, allOrders, current,
    isLoading, isCreating, error, meta,
    placeOrder, cancel, fetchAll, getById, changeStatus,
